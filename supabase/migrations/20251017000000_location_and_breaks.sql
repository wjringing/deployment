/*
  ...
  - Real-time staff location tracking during service
  - Automated break scheduling with coverage management
  ...
  Real-time tracking of staff station assignments
  - `id` (uuid, primary key) - Unique identifier
  - `staff_id` (uuid, foreign key) - Staff member
  - `deployment_id` (uuid, foreign key) - Associated deployment
  - `current_position` (text) - Current station/position
  - `assigned_area` (text) - Assigned area
  - `status` (text) - Status: working, on_break, offline
  - `started_at` (timestamptz) - When started at position
  - `last_updated` (timestamptz) - Last location update
  - `notes` (text) - Special notes
  - UNIQUE constraint on staff_id (one location per staff member)

  #### staff_location_history
  Historical record of staff movements
  - `id` (uuid, primary key) - Unique identifier
  - `staff_id` (uuid, foreign key) - Staff member
  - `deployment_id` (uuid, foreign key) - Associated deployment
  - `position` (text) - Station worked
  - `started_at` (timestamptz) - Started timestamp
  - `ended_at` (timestamptz) - Ended timestamp
  - `duration_minutes` (integer) - Time at station (auto-calculated)
  - `moved_by_staff_id` (uuid, foreign key) - Manager who moved
  ...
  - `shift_type` (text) - Day Shift, Night Shift
  - `break_type` (text) - rest_break, meal_break, rest_period
  - `break_duration_minutes` (integer) - Break length
  - `scheduled_start_time` (text) - Planned start time (HH:MM)
  ...
  - `coverage_staff_id` (uuid, foreign key) - Who covers position
  - `uk_compliance_checked` (boolean) - Labor law verified
  - `notes` (text) - Break notes
  - `created_at` (timestamptz) - Creation timestamp

  #### break_coverage_gaps
  Tracks positions with insufficient coverage during breaks
  - `id` (uuid, primary key) - Unique identifier
  - `date` (date) - Service date
  - `shift_type` (text) - Day Shift, Night Shift
  - `position` (text) - Uncovered position
  ...
  - Add `is_on_break` (boolean) to deployments table
  - Add `break_schedule_id` (uuid) to deployments table
  - Add `current_location_id` (uuid) to staff table
  ...
  - Indexes on date and shift_type
  - Composite indexes for common queries
  - Auto-calculation triggers for duration
*/
...
-- Auto-update last_updated timestamp for locations
...
-- Auto-calculate duration for location history
...
-- Auto-update deployment break status when break starts/ends
