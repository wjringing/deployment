/*
  ...
  ### Feature 5: Labor Hours vs. Sales Real-Time Calculator
    - `labor_sales_snapshots`
      ...
      - `shift_type` (text) - Type of shift (opening, mid, closing)
      - `snapshot_time` (timestamptz) - When snapshot was taken
      - `total_labor_hours` (numeric) - Total staff hours deployed
      - `total_sales` (numeric) - Total sales at snapshot time
      - `labor_percentage` (numeric) - Labor cost as % of sales
      - `target_labor_percentage` (numeric) - Target labor percentage
      - `variance` (numeric) - Difference from target
      - `staff_count` (integer) - Number of staff working
    ...
    - `labor_sales_targets`
      ...
      - `target_labor_percentage` (numeric) - Target labor %
      - `min_labor_percentage` (numeric) - Minimum acceptable %
      - `max_labor_percentage` (numeric) - Maximum acceptable %
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)
  ...
  ### Feature 6: Shift Performance Scorecard
    - `shift_performance_scorecards`
      ...
      - `shift_type` (text) - Type of shift
      - `sales_target` (numeric) - Sales target for shift
      - `actual_sales` (numeric) - Actual sales achieved
      - `sales_score` (integer) - Score 0-100
      - `labor_efficiency_score` (integer) - Score 0-100
      - `speed_of_service_score` (integer) - Score 0-100
      - `quality_score` (integer) - Score 0-100
      - `checklist_completion_score` (integer) - Score 0-100
      - `overall_score` (integer) - Average of all scores
      - `shift_manager` (text) - Manager on duty
    ...
    - `performance_metrics`
      - `id` (uuid, primary key)
      - `scorecard_id` (uuid, foreign key) - Links to scorecard
  ...
  ### Feature 7: Station Position Mapping
    - `stations`
      - `id` (uuid, primary key)
      - `station_name` (text) - Name of station (e.g., Front Counter, Drive-Thru)
    ...
    - `station_position_mappings`
      - `id` (uuid, primary key)
      - `station_id` (uuid, foreign key) - Links to station
      - `position` (text) - Position name (from existing system)
      - `is_primary` (boolean) - Is this the primary position for station
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)
  ...
  ### Feature 8: Enhanced Intelligent Deployment Auto-Assignment
    - `auto_assignment_rules`
      ...
      - `rule_type` (text) - Type: position_based, skill_based, seniority_based, etc.
    ...
    - `assignment_history`
      - `id` (uuid, primary key)
      - `deployment_id` (uuid, foreign key) - Links to deployment
      - `staff_id` (uuid, foreign key) - Links to staff
      - `assigned_position` (text) - Position assigned
      - `assignment_method` (text) - How assigned (auto, manual)
      - `rule_applied` (text) - Which rule was used
      - `assignment_score` (numeric) - Quality score of assignment
      - `created_at` (timestamptz)

  ## 2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users to manage their own data
    - Add policies for viewing historical data

  ## 3. Indexes
    - Add indexes on foreign keys for performance
    - Add indexes on date fields for reporting
...
-- RLS Policies for labor_sales_snapshots

CREATE POLICY "Users can view labor sales snapshots"
  ON labor_sales_snapshots FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert labor sales snapshots"
  ON labor_sales_snapshots FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update labor sales snapshots"
  ON labor_sales_snapshots FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete labor sales snapshots"
  ON labor_sales_snapshots FOR DELETE
  TO authenticated
  USING (true);

-- RLS Policies for labor_sales_targets

CREATE POLICY "Users can view labor sales targets"
  ON labor_sales_targets FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert labor sales targets"
  ON labor_sales_targets FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update labor sales targets"
  ON labor_sales_targets FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete labor sales targets"
  ON labor_sales_targets FOR DELETE
  TO authenticated
  USING (true);

-- RLS Policies for shift_performance_scorecards

CREATE POLICY "Users can view performance scorecards"
  ON shift_performance_scorecards FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert performance scorecards"
  ON shift_performance_scorecards FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update performance scorecards"
  ON shift_performance_scorecards FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete performance scorecards"
  ON shift_performance_scorecards FOR DELETE
  TO authenticated
  USING (true);
...
CREATE POLICY "Users can update performance metrics"
  ON performance_metrics FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete performance metrics"
...
-- RLS Policies for station_position_mappings

CREATE POLICY "Users can view station position mappings"
  ON station_position_mappings FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert station position mappings"
  ON station_position_mappings FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update station position mappings"
  ON station_position_mappings FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete station position mappings"
  ON station_position_mappings FOR DELETE
  TO authenticated
  USING (true);

-- RLS Policies for auto_assignment_rules

CREATE POLICY "Users can view auto assignment rules"
  ON auto_assignment_rules FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert auto assignment rules"
  ON auto_assignment_rules FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update auto assignment rules"
  ON auto_assignment_rules FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete auto assignment rules"
...
CREATE POLICY "Users can insert assignment history"
  ON assignment_history FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Users can update assignment history"
  ON assignment_history FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Users can delete assignment history"
...
-- Seed default labor sales targets
