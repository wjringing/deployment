/*
  # Safe Production Database Sync

  This migration safely syncs production with development by:
  1. Dropping existing policies before recreating them
  2. Using IF NOT EXISTS for all table creation
  3. Checking for existing columns before adding them
*/
...
DROP POLICY IF EXISTS "Authenticated users can view labor_sales_snapshots" ON labor_sales_snapshots;
DROP POLICY IF EXISTS "Authenticated users can manage labor_sales_snapshots" ON labor_sales_snapshots;
DROP POLICY IF EXISTS "Authenticated users can view labor_sales_targets" ON labor_sales_targets;
DROP POLICY IF EXISTS "Authenticated users can manage labor_sales_targets" ON labor_sales_targets;
DROP POLICY IF EXISTS "Authenticated users can view shift_performance_scorecards" ON shift_performance_scorecards;
DROP POLICY IF EXISTS "Authenticated users can manage shift_performance_scorecards" ON shift_performance_scorecards;
DROP POLICY IF EXISTS "Authenticated users can view performance_metrics" ON performance_metrics;
DROP POLICY IF EXISTS "Authenticated users can manage performance_metrics" ON performance_metrics;
DROP POLICY IF EXISTS "Authenticated users can view auto_assignment_rules" ON auto_assignment_rules;
DROP POLICY IF EXISTS "Authenticated users can manage auto_assignment_rules" ON auto_assignment_rules;
DROP POLICY IF EXISTS "Authenticated users can view assignment_history" ON assignment_history;
DROP POLICY IF EXISTS "Authenticated users can manage assignment_history" ON assignment_history;
...
DROP TRIGGER IF EXISTS update_labor_sales_snapshots_updated_at ON labor_sales_snapshots;
CREATE TRIGGER update_labor_sales_snapshots_updated_at
  BEFORE UPDATE ON labor_sales_snapshots
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_labor_sales_targets_updated_at ON labor_sales_targets;
CREATE TRIGGER update_labor_sales_targets_updated_at
  BEFORE UPDATE ON labor_sales_targets
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_shift_performance_scorecards_updated_at ON shift_performance_scorecards;
...
DROP TRIGGER IF EXISTS update_auto_assignment_rules_updated_at ON auto_assignment_rules;
